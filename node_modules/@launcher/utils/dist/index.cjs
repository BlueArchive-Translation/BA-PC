...
var fsExtra = /*@__PURE__*/getDefaultExportFromCjs(libExports);

function removeFiles(dir, deleteList = [], pc) {
    // 删除文件
    deleteList.forEach((fileItem, index) => {
        ...
        pc === null || pc === void 0 ? void 0 : pc(Math.round(((index + 1) / deleteList.length) * 100));
    });
}
...
const getAuthHeader = function (options) {
    const head = {
...
function readManifestFile(manifestPath) {
    ...
    delete result.vc;
    delete result.files;
    const infoVC = data === null || data === void 0 ? void 0 : data.vc;
    ...
    const validFiles = ((_a = data === null || data === void 0 ? void 0 : data.files) === null || _a === void 0 ? void 0 : _a.filter((item) => {
        const itemData = Object.assign({}, item);
        delete itemData.vc;
        const checkVC = getObjectHash(itemData);
        return item.vc === checkVC;
    })) || [];
    return Object.assign(Object.assign({}, result), { vc: data === null || data === void 0 ? void 0 : data.vc, files: validFiles });
}
// 读取配置
const readGameConfigFile = (gameConfigPath) => {
    const origin = {};
    try {
...
const getDownloadTmpName = (name) => `${name}.tmp`;
const getDownloadOriginName = (tmpName) => tmpName.replace(/\.tmp$/, "");
 ...
 * @returns string
 */
function timeToReadable(time, unit = "ms") {
 ...
 * @returns string
 */
function bytesToSize(bytes, fractionDigits = 2) {
    if (!bytes)
        return "0KB";
    ...
    if (i >= sizes.length)
        i = sizes.length - 1;
    return `${parseFloat((Number(bytes) / Math.pow(1024, i)).toFixed(fractionDigits))}${sizes[i]}`;
}
...
const setProxyEnvironment = (proxyInfo) => {
    if (proxyInfo.url === "") {
...
function isExeRunning(exeName) {
    return new Promise((resolve, reject) => {
        node_child_process.exec(`chcp 65001 && tasklist /FO CSV /NH /FI "IMAGENAME eq ${exeName}"`, (err, stdout, stderr) => {
            if (err || stderr) {
...
function killEXE(exeName) {
    return new Promise((resolve, reject) => {
        node_child_process.exec(`chcp 65001 && taskkill /IM "${exeName}" /F /T`, (err, _stdout, stderr) => {
            if (err || stderr) {
                return reject(err || stderr);
            }
            return resolve(true);
...
exports.getDownloadOriginName = getDownloadOriginName;
exports.getDownloadTmpName = getDownloadTmpName;
exports.getObjectHash = getObjectHash;
exports.getWindowsSystemProxy = getWindowsSystemProxy;
exports.isExeRunning = isExeRunning;
exports.killEXE = killEXE;
exports.readGameConfigFile = readGameConfigFile;
exports.readManifestFile = readManifestFile;
exports.removeFiles = removeFiles;
exports.setProxyEnvironment = setProxyEnvironment;
exports.timeToReadable = timeToReadable;
